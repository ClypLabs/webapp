...
// Google's four-colour G, for accounts that still hold a Google sign-in.
function GoogleIcon() {
  return (
    <svg aria-hidden="true" viewBox="0 0 24 24" className="h-5 w-5 shrink-0">
      ...
      <path fill="#EA4335" d="M12 4.8c1.8 0 3.3.6 4.6 1.8l3.4-3.4A11.5 11.5 0 0 0 12 0 12 12 0 0 0 1.3 6.6l4 3.1c.9-2.8 3.6-4.9 6.7-4.9Z" />
    </svg>
  );
}
...
export default function AccountPage() {
  ...
  // The "Add an account" card goes once there is nothing left in it.
  const canAddAccount = availableSocials.length > 0 || !xboxConnected || onlyDiscord;
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // A dropped connection throws rather than returning an error, and used
      // to leave the button on "Working…" for good.
      setError("ClypDat could not be reached. Check your connection and try again.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function socialSignIn(provider: SocialProvider) {
    ...
    try {
      ...
      // Success navigates away, so the busy state only needs clearing on failure.
      if (result.error) {
        setError(result.error.message ?? `${socialProviderName(provider)} sign-in is not configured yet.`);
        setSignInBusy(false);
      }
    } catch {
      setError(`${socialProviderName(provider)} sign-in could not be started. Try again.`);
      setSignInBusy(false);
    }
  }
  ...
  // A full navigation rather than router.push: it drops the client session
  // cache and every piece of state on this page, so nothing from the account
  // just signed out of can show for a frame.
  ...
  return (
    <main className="flex min-h-screen items-center justify-center px-6 py-20">
      <section className="w-full max-w-md rounded-3xl border border-white/10 bg-white/[0.04] p-8 shadow-2xl shadow-black/30">
        ...
        <button type="button" disabled={signInBusy} onClick={googleSignIn} className="mt-4 w-full text-center text-sm text-zinc-400 underline-offset-4 hover:text-zinc-200 hover:underline disabled:cursor-wait">
