...
export default function CopyCommand({ command }: { command: string }) {
  ...
  return (
    <div className="flex items-center gap-3 border border-rule bg-panel py-2 pl-4 pr-2 text-left">
      <span aria-hidden className="select-none font-mono text-sm text-faint">
        &gt;
      </span>
      <code className="min-w-0 flex-1 overflow-x-auto whitespace-nowrap font-mono text-sm text-paper [scrollbar-width:none]">
        {command}
      </code>
      <button
        type="button"
        ...
        className="slate flex h-8 shrink-0 items-center gap-1.5 border border-rule-strong px-3 text-dim transition-colors hover:border-faint hover:text-paper"
      >
        {copied ? (
          <>
            <svg aria-hidden viewBox="0 0 24 24" className="h-3.5 w-3.5 text-saved" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round">
