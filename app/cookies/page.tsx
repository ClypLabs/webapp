...
export default function CookiesPage() {
  return <PolicyPage title="Cookie Notice" sections={[
    { title: "Authentication cookies", content: <p>ClypDat uses cookies only when you use an account. Better Auth sets session and sign-in-flow cookies so the service can keep you signed in and protect sign-in requests. One of them holds a signed copy of your session for up to five minutes, so pages do not look you up in the database on every request. These cookies are necessary for the account feature. Signing out clears them.</p> },
