...
export async function saveSupportReport(userId: string, input: SupportInput): Promise<"saved" | "duplicate" | "conflict" | "full"> {
  ...
  try {
    ...
    const prior = await client.query<{ user_id: string }>("SELECT user_id FROM clypdat_support_reports WHERE id = $1", [input.id]);
    if (prior.rows[0]) {
      await client.query("COMMIT");
      return prior.rows[0].user_id === userId ? "duplicate" : "conflict";
    }
    ...
    await client.query(`INSERT INTO clypdat_support_reports (id, user_id, message, version, build, bundle, bytes)
      VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      [input.id, userId, input.message, input.version, input.build, sealSupportBundle(input.id, input.bundle), input.bundle.length]);
    await client.query("COMMIT");
    return "saved";
...
export async function listSupportReports(): Promise<SupportReport[]> {
  ...
  const result = await pool.query(`SELECT r.id, r.message, r.version, r.build, r.bytes, r.resolved,
    r.created_at AS "createdAt", r.expires_at AS "expiresAt", u.name, u.email
    FROM clypdat_support_reports r JOIN "user" u ON u.id = r.user_id
